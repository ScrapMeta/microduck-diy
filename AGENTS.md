...
| 改什么 | 谁改 | 怎么算数 |
|---|---|---|
| 技能 `SKILL.md`（含职能 · 红线 · 骨架） | **中立会话执笔** —— 角色会话只**提议**：干完把「该改哪句」交回来，不自己落笔 | 改完停下，**等 Human 确认** |
| 规则 `AGENTS.md` | **中立会话**（不唤起职能角色） | 改完停下等我确认 |
| wiki（`concepts` · `log` · `tasks` · 报告） | **干活的那个角色写** | 按「角色 → 必写记录」直接写，不用逐次问 |

> **技能可以进化，但执笔的永远是中立会话** —— 职能角色只提议、不落笔。**谁能改自己的红线，等于没有红线。**
> 本表三行同一个原则：**规则不写在被约束者手里**。
...
**手册随干活长大** —— 技能随活变，但**执笔的是中立会话**：角色会话干完把「该改哪句」交回来（见上一节）。
