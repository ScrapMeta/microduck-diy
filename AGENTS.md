...
**台账**（`wiki/tasks.md`）—— **编号 · 四态看板（`待办` / `执行` / `阻塞` / `完成`）· 三条上限 · 完成 / 关闭语义**按**通用方式**写在那页「**通用约定**」（可整段移植到别的工程）；**本文件不复制**。

- **超限即停，不是提醒** —— 触到任一条上限，agent 直接拒绝，并在回复里点名超限项。
- **台账不属任何单一角色** —— **谁干的活谁能开 / 更新 / 关自己的任务**；**push 与 Issue 归档只有 pm**。
- **未归档关闭行超限 → 先收尾再干别的**：`push main` → 关闭行归入 Issue → 从 `tasks.md` 删掉该行。
