...
diy 仓：`imu_to_dxl/` · `image/` · `cad/` · `wiki/`。治理：`docs/agent-governance.md`（**v0.9：GitHub 基础工程 + llm-wiki；只常驻 pm；无 handoff**）；入口 `AGENTS.md`。`microduck_ros2` **不在本 wiki 主轴**（并入 software 范畴，规格见 [[ros2-migration-plan]]）。
