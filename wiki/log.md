...
- **由来（Human）**：问「看一下目前的任务，合理排序，如果任务太多怎么办」—— 选定「全部改」＋「给台架执行顺序」
- **关键发现（这才是「太多」的真因）**：「进行中」6 条里 **5 条挤在同一个瓶颈** —— 都要**上电**，而红线 6 规定只有 Human 能签。所以那不是 5 件事，是**一件接一件**的事；给它们排先后等于给串行队列重新编号。**排顺序改变不了串行，只有分类能。**
- **改法 ①「阻塞源」列**（🖐 台架 / 🟢 agent 可独立 / ⏳ 等货+日期 / ✋ 等 Human / ⚠️ 待确认）：原表 18 行是平的，每次打开都得逐行重判能否开工；现在一眼看出「现在能动的是哪几条」
- **改法 ② WIP 上限 3**：「进行中」6 → **3**（按 [[index]] 现行优先级 ①②③ 取：整机装配供电 · 官方软件联调 · 新焊 3 块 HAT），其余**退回待办不删**。一个人 ＋ 一个台架推不了 6 条 —— 多认领的只会让「进行中」变成自我欺骗
- **改法 ③ 砍伪任务**：原「本轮物料事实落页」**整行删掉**（没有自己的触发条件，是各宿主任务的收尾动作，验收已分发：降压模块→整机装配 · 闲鱼批→舵机清点 · ToF/喇叭→到货测试）；「喇叭/ToF 到货先测」＋「喇叭出声孔 · ToF 外形实测」**合并**（同到货日、同时能做，拆两行只是让台账更长）。**18 行 → 16 行**
- **改法 ④ 收窄无界验收**：「契约对齐」原验收「[[opensource-coverage]] 无未决缺口」是**开口**、永远勾不掉 → 改为「先出缺口清单 → 每条给判定：对齐 / 接受差异（附理由）/ 待定（附阻断条件）→ 收口时不得只剩待定」
- **新增规则**：台账页面顶部写明「**无界验收必须先收窄才准立项**」
- **新拆项**：`replica0908` 原一行含「焊接 ＋ U2D2 冒烟」，两者阻塞源不同（🟢 vs 🖐）→ 标注拆分，焊接可先干
- **待 Human 补事实**：「结构可干装配」标 ⚠️ —— 轴承 11＋3 已购，但**打印件是否已打台账无记录**，不猜
- **台架执行顺序（另附，未落页）**：① 整机装配＋供电链路（定母线，后面全靠它）→ ② 直供对照（共用接线，一次拆装做完两个）→ ③ HAT TTL 写总线值 ＋ `0x55` → ④ 新焊 3 块 HAT（**焊接不占台架，作等待间隙的填充工作**）→ ⑤ 官方软件联调（需 ①④ 均过）
- Updated: [[tasks]] · `log.md`

## [2026-09-19] pm | 台账：加编号 · 拆出已完成 · 删 3 条 · 关 1 条（16 → 12 活跃）
- **由来（Human 指令）**：删 `replica0908 焊接` · `回收上游 microduck-replica 那 9 个提交` · `契约对齐 DXL/IMU/策略`；关 `结构可干装配`；加任务编号；已完成另存；保持活跃数量可控
- **新增编号 `T-01`–`T-12`** —— **一经分配不再变动、不复用**；任务完成后**编号随行**搬进 `tasks-done.md`。用途：`log.md`、页间引用、对话都能指认同一条任务，不用抄任务名。编号自 2026-09-19 启用，**此前条目无编号**（靠 `出处` 追溯）—— 不回溯补号，避免纯churn
- **已完成另存 → 新建 [`tasks-done.md`](wiki/tasks-done.md)**（Human 问「是否另存」，答复：**是**）。理由：已完成的 10＋ 行占了 `tasks.md` 近一半体量，而它们**只在回看时才需要**；活动台账要的是「现在欠什么」。新页**只追加不删**，行一旦落地不再改动（原行永远在 git 历史里）
- **关闭 ≠ 完成（写进新页）**：新页分**两张表** —— 「已完成」（过验收）／「已关闭（未完成 · 不再追）」。两者都搬过来但落不同表，**免得后人把「没做」读成「做完了」**。关闭理由必须写清；淡出的留在 `tasks.md`「等外部」，不算关闭
- **关闭 `结构可干装配`** → **已完成**（Human 2026-09-19 确认「打印件 / 紧固件 / 轴承 11＋3 / XL330 均装得上」）。⚠️ 两点如实记：① `件数是否已并入` [[mechanical-bom-rl]] **未核实** ② [[diy-milestones]] 对应行**未改勾**（同行含未完成的机身 IMU 竖装 → T-06），**不改** [[diy-milestones]]（该页属 structure 地盘，且该行本就关不掉）
- **删 3 条**（Human 指定）：`replica0908 焊接` · `回收上游 9 提交` · `契约对齐`。**只在本页删，不抹历史** —— 原文在 git 历史与旧版 `tasks.md` 里，本页留可追溯边界
- **两条上限（防台账自己长胖）** —— 原「WIP ≤ 3」保留，**新增活跃上限 ≤ 12**（进行中 ＋ 待办 ＋ 等外部）：**满了要开新的，就先关掉一条**。当前 **3 ＋ 7 ＋ 2 = 12，正好到顶**
- **活跃数量**：16 → **12**（删 3 ＋ 关 1）
...
## [2026-09-19] governance | 收回技能执笔权：规则不写在被约束者手里

- **由来（Human 纠正）**：上一轮我把「`tasks.md` / `index.md` / `log.md` / `pm/SKILL.md` 是 pm 地盘」当成**权限**，
  写下「越领地」并说「等 pm 会话确认」。Human 驳回：**领地表的约束对象是角色会话，不是权限系统**；
  规则改动一律由**人工拉起的中立会话**做；pm 只是规则定义出的一个职能角色，**不能反过来改规则**
- **修的是规则不是措辞**：`AGENTS.md` 原「改规则表」第 1 行写着 `SKILL.md` = **角色会话自己改**（技能要能自己进化）
  —— 这等于让 pm 会话自己改 `pm/SKILL.md` 里的**职责与红线**，即「被约束者持有笔」。已改为**中立会话执笔，角色会话只提议**
- **改法**：① 改规则表第 1 行改为中立会话执笔 ＋ 等 Human 确认 ② 表下注改为「**规则不写在被约束者手里**」③
  「手册随干活长大」改为「技能随活变，但执笔的是中立会话」④ 领地表下加**约束对象声明**：它管的是角色会话，
  **Human 点名要改的路径不存在「越领地」** ⑤ `pm/SKILL.md` 职责 5 与红线同步（pm 只提议、不落笔、更不改自己的红线）
- **未改**：`wiki/_archive/governance/README.md` 里「能自己进化」的旧说法 —— 归档页不删不改
- Updated: `AGENTS.md`（改规则表 · 手动随活变 · 领地约束对象）· [[pm 技能|microduck-pm]]（职责 5 ＋ 红线）· `log.md`

## [2026-09-19] governance | 订正上一条：技能自进化合法，规则统一由 Human 拉起中立会话

- **由来（Human 二次纠正）**：「技能就是能自己进化，我也可以拉起中立 agent 改，不是绝对的，但是**整体规则制定统一由我拉起中立 agent 干**」
  —— 上一条把技能执笔权**全部**收给中立会话，收过头了
- **订正后的口径（三条）**：① **技能可自进化** —— 角色会话改自己的手册合法；Human 也可点中立会话改，两条路都通
  ② **技能不是规则源** —— 与 `AGENTS.md` 冲突时以 `AGENTS.md` 为准；**扩权**（放宽红线 · 绕开确认 · 改改规则表）不属自进化，仍须 Human 拉起中立会话点头
  ③ **规则制定统一由 Human 拉起中立会话** —— 职能角色不碰 `AGENTS.md`
- **改法**：`AGENTS.md` 改规则表第 1 行复原为「角色会话自进化 · 或 Human 点中立会话改」，注改为「技能能自己进化 —— 但技能不是规则源」；
  「手册随干活长大」复原为可自改；`pm/SKILL.md` 职责 5 与红线同步（规则不碰 · 技能可自进化但不得冲突 / 扩权）
- **保留（上一条的成果）**：领地表下的**约束对象声明** —— 它管的是**角色会话**，不是权限系统；Human 点名的路径，中立会话照做
- Updated: `AGENTS.md`（改规则表 · 手册随干活长大）· [[pm 技能|microduck-pm]]（职责 5 ＋ 红线）· `log.md`

## [2026-09-19] governance | 台账改通用口径（便于跨工程推广）＋ 收敛重复上限

- **由来（Human 指令）**：「push 留给 pm 干，你再检查一下规则，**tasks 我建议用通用规则，便于我统一规范推广到各个工程治理**，记号、规则、状态用通用方式和说法，其他可以」
- **`tasks.md` 新增「通用约定」节**（可整段搬到别的工程，只换「本工程实例」一列）：
  ① **状态 5 个**：进行中 · 待办 · 等外部 · 已完成 · 已关闭；活跃 = 前三，终态不计 ② **编号** `T-nn` 两位定宽、不复用、随行走
  ③ **阻塞记号**改成「通用含义 ＋ 本工程实例」两列（🖐 需人工在场/签核 · 🟢 可自动推进 · ⏳ 等外部 · ✋ 等人工动作 · ⚠️ 待确认才能定）
  ④ **规则 4 条**：只追加不改旧行 · 开＋关同页 · 关闭≠完成 · 准入（无界验收先收窄）⑤ **上限 3 条**表格化（量 / 上限 / 超了怎么办）
- **检查规则发现两处真问题（已修）**：① `README.md`「不建 PR · **不开 Issue** · 不建标签」与新 GitHub 节冲突 → 改为「Issue 只作 pm 归档」
  ② `tasks.md` 标题「三条上限」下挂了 4 条（第 4 条「无界验收」是**准入**不是上限）→ 上限独立成 3 条表、准入移进规则 4
- **收敛重复正文**：台账上限原先在 `AGENTS.md` 与 `tasks.md` 各一份（＋ `pm/SKILL.md` 一份数字）→ **正文只留 `tasks.md`「通用约定」**；
  `AGENTS.md` 只留「超限即停」的**执行力度** ＋ 台账非独占 / push·Issue 归 pm，`pm/SKILL.md` 改为指路
- **留待 Human 定（未改）**：`microduck-software/SKILL.md` 写 `microduck_ros2/`「需自行 push」—— 兄弟仓是**另一个仓**，
  「只有 pm push」是否覆盖它，规则现文只说了本仓 `main`；不确定就没动
- Updated: [[tasks]]（新增通用约定 · 上限表 · 准入移入规则）· `AGENTS.md`（台账节改指路 ＋ 收敛）· `README.md`（Issue 措辞）· [[pm 技能|microduck-pm]]（上限改指路）· `log.md`

## [2026-09-19] governance | 台账改四态看板：撤 5 记号，关闭并入完成

- **由来（Human）**：「通用的意思就是不要那么多复杂标记，不就是**待办、执行、阻塞、完成**（把关闭合了，内容说明就行了）吗？这是不是最标准的看板状态？」
  —— 确认：是。`To Do → In Progress → Blocked → Done` 就是通用 kanban 四列；加 Backlog / Review 列或给阻塞加类型标签都是变体，通用的是这四态
- **撤掉 5 个阻塞记号**（🖐 🟢 ⏳ ✋ ⚠️）—— 它们编码的两件事已被状态吸收：
  「能不能动手」= 在 `待办` 还是 `阻塞`；「为什么动不了」= `阻塞` 行的**阻塞原因**一句白话。记号没了，状态机仍是 4 个
- **关闭并入完成** —— `已完成` / `已关闭` 两张终态表合一张「完成」，靠行内**结果说明**区分：真做完写「**完成**：结论 ＋ 落点」，
  判定不做写「**关闭**：不采用 / 被谁吸收（理由必须写清）」。终态 2 个变 1 个，语义不丢（原来靠分表，现在靠词）
- **活跃口径** = 待办 ＋ 执行 ＋ 阻塞（三态）；上限仍 3 条：执行 ≤ 3 · 活跃 ≤ 12 · 未归档完成行 ≤ 5
- **本次状态迁移（我的判断，可推翻）**：原「待办」里 4 条实质推不动 → 移入 `阻塞` 并写明原因
  —— T-04 / T-05（需本人到台架）· T-09（卡提权）· T-10（等寄出）；原「等外部」T-11 / T-12 并入 `阻塞`（等货）
  → 现 **待办 3 · 执行 3 · 阻塞 6 = 活跃 12**，**正好到顶**
- **⚠️ 迁移后暴露两件事（未处理，交 Human）**：① **活跃 12/12 到顶** —— 按规则此后不许开新任务，须先完成 / 关闭一条
  ② **完成表 11 行 > 未归档完成行 ≤ 5** —— 其中 10 条是编号启用前的旧行（本就无号），触发 pm 收尾；
  且「归档成 11 个 Issue」还是「一批一个 Issue」没定
- Updated: [[tasks]]（四态重写 · 撤记号 · 关闭并入完成）· `AGENTS.md`（台账节措辞）· [[pm 技能|microduck-pm]]（开任务口径）· `log.md`

## [2026-09-19] governance | 归档粒度定案：一批合成一个 Issue

- **由来（Human 决定）**：「归档粒度：**一批合成一个 Issue**。我现在就安排 pm 清完成任务 push」
  —— 否掉了「11 条各开一个 Issue」的做法；理由与上次收敛台阶一致：**逐条开 = 往 GitHub 倒 11 条噪声**，而这一批的意义就是「编号启用前的历史清账」
- **写进规则正文（两处）**：① `tasks.md` 上限表「未归档完成行 ≤ 5」的超限动作 → `push main` → 完成行**一批合成一个 Issue** → 从本页删掉**该批**行
  ② `pm/SKILL.md` 收尾步骤 1 同步
- **顺手订正两处过时措辞**：`pm/SKILL.md` ① 收尾步骤里的「已完成 / 已关闭」（分表时代说法）→ 「完成」② 收口步骤里的 ✅ / ❌ 记号 → 终态都落「完成」表、靠结果说明区分
- **交接 pm**：本轮共 **7 个本地 commit** 待 pm 推；收尾批次 = 完成表 11 行（T-01–T-12 编号体系启用前的旧行，本就无号）
- Updated: [[tasks]]（上限表归档粒度）· [[pm 技能|microduck-pm]]（收尾 + 收口措辞）· `log.md`

## [2026-09-19] archive | 完成行清账：11 行 → Issue #12（开完即关）

- **push**：`d2e5e92..9010e98`（`main`）—— 6 个本地 commit（台账单页化 · 技能执笔权订正 · 四态看板 · 归档粒度定案）上远端
- **归档**：[#12](https://github.com/ScrapMeta/microduck-diy/issues/12) —— 完成表 **11 行一批合成一个 Issue**，正文含逐条**结论 ＋ 出处**，**创建后即关**（`completed`）
  - **无标签 · 无 Milestone · 不作路由** —— 按 `AGENTS.md`「Issue 只作归档」，与冻结的历史 #1–#11 不混
- **清理**：该批行**已从 `wiki/tasks.md` 删除**（git 历史留痕）→ 完成表回到 **0 / 5**，「未归档完成行」超限解除
- **⚠️ 未解除**：**活跃仍 12/12 到顶**（待办 3 · 执行 3 · 阻塞 6）→ 按规则**此后不许开新任务**，须先完成 / 关闭一条
- **校验**（收口前）：`wiki_lint.py` 61 页 0 error / 3 warning（3 个已知欠拆页 → **T-08**）· `refs_lint.py` 76 文件 342 引用 0 error
- 遗留（不属本次范围，未动）：归档行里的「件数是否已并入 [[mechanical-bom-rl]] 未核实」· `diy-milestones` 对应行未改勾 → 可归 **T-06**
- Updated: [[tasks]]（完成表清空）· `log.md`

## [2026-09-19] governance | 台账撤数量上限：标准四态看板 ＋ 超多人工收拢

- **由来（Human 指令）**：「一切从简，看板标准状态，四个状态表显示，没有数量限制，多了人工处理」
- **撤掉三条数量上限**（原 执行 ≤ 3 · 活跃 ≤ 12 · 未归档完成行 ≤ 5）—— 台账**不设上限**：想开就开，**不因条数多而拒绝**；**太长由人工收拢**（`push main` → `完成` 行一批归档 Issue → 从本页删掉该批行），agent 不替人砍任务
- **四态看板保持标准** —— `待办` / `执行` / `阻塞` / `完成`（`To Do` / `In Progress` / `Blocked` / `Done`），**四张表逐一对应**；顺手撤掉「活跃」这个只服务上限的口径
- **收敛重复正文**（同一句话只留一个正文）：`tasks.md`「通用约定」是数量口径的**唯一正文**；`AGENTS.md` 只留「不设上限 · 太长人工收拢」＋ 台账非独占 / push·Issue 归 pm；`pm/SKILL.md` 职责 4「上限把关」→「不做数量把关」
- **保留不变** —— 编号 `T-nn` 不复用 · 开 ＋ 关同页 · 完成 ≠ 关闭（靠结果说明区分）· 准入（无界验收先收窄）· 归档流（pm 独有）
- Updated: [[tasks]]（撤上限 · 状态节改标准看板 · 表头去计数）· `AGENTS.md`（台账节 · 记录约定表）· [[pm 技能|microduck-pm]]（职责 4）· `log.md`

## [2026-09-19] governance | 台账不记归档动作：拆掉「记归档的那一行」自指

- **由来（Human）**：「task 里的那条完成的任务是什么，pm 干的 push 和 issue 这个就不要再记录了，这是个死循环吗？」
- **查清那条不是任务** —— `完成` 表里 `| — | *（本批已归档）* | 上一批 **11** 行于 2026-09-19 归档 → [#12] | — |`：无编号、无验收，是**归档备注占位行**，记的是编号体系启用前那批历史旧行的清账
- **确认是结构性自指（不是自动循环，但收敛不了）**：归档动作的记录**写在会被归档的那张表里** → ① pm 推送时把 `完成` 行归档并删行 ② 删时留下「本批已归档 → #xx」备注 ③ 该备注自己也在 `完成` 表里 → 成为下一轮的待归档行 → **每轮清账再生一条元数据行**。根因：**把「归档的记录」放进了会被归档的表**
- **定案（Human）**：**保留归档**（pm 推送时把 `完成` 行一批归入 Issue 后从本页删掉）；**归档这个动作只记 `log.md`（只追加），不在 `tasks.md` 留任何占位行**
- **顺手修掉一处规则自相矛盾** —— `AGENTS.md` 记录约定表写「关闭行**留在本页**」，GitHub 节写「推送时**从本页删掉**」，两条同时在；改为「终态行落 `完成` 表 · 关闭理由必须写清（pm 推送时归档后删除，见 GitHub）」
- Updated: [[tasks]]（`完成` 表去占位行 · 补归档口径与自指说明）· `AGENTS.md`（记录约定表 · GitHub 推送节）· [[pm 技能|microduck-pm]]（职责 1）· `log.md`
## [2026-09-19] pm | T-08 结项：`hat-solder-kit` 简化到 194 行；另两页转「已批准例外 · 不拆页」

- **由来（Human）**：T-08（3 页超 200 行）→ 先给「`hat-solder-kit` 无用删除，其他几个有用保留不分页」；预检后改定为「**改成尽量简化，保留关键点**」（不删、不归档），另两页「**算永久例外**」
- **预检（红线 3 · 先列范围未动手）**：`hat-solder-kit` 在役引用 **8 处** —— [[index]] 导航 · [[tasks]]（T-08 行 ＋ T-03/T-12 验收）· [[board-hat]] · [[hat-dxl-bus-debug]]（5 处）· [[diy-milestones]]（4 处）· [[diy-bom]] · [[elec-hat-lcsc-order-2026-09-05]] · `microduck-hardware` 手册。**真删要同批改这 8 处**，且 **T-03「新焊 3 块 HAT」的 §4 流程会悬空**；归档虽零改引用（`wiki_lint` 的链接表**含 `_archive/`**），但 Human 要的是**简化**
- **简化（399 → 194 行 · −205）**：**章节号一个没动**（`§3.1–§3.4` · `§4.0–§4.9` · `§6.1–§6.4` —— T-03/T-12/board-hat/hat-dxl-bus-debug/diy-milestones 都**按号引用**），**数值一个没丢**（183 / 215 °C · 138 °C 膏 · 风枪 240–260 °C · `+5V` 5.0 V / `+1V8` 1.8 V · `0x18` · 净空 35×25×7 mm · 485 必留 R29/C15）
  - 删的是重复与冗余：§2 逐值位号枚举 ＋ LCSC 参考号（**回指真源 BOM CSV** 与订单页）· §3.3 掉件论证长文 · §4.3 数值表并成一行 · §4.6 / §4.7 举例收敛
  - **原 §3.4「目检」并入 §3.2 步 3**（桥连重灾区 U2/U8 ＋ 酒精清洁）→ 短路测 **§3.5 改 §3.4**（已核：无页外引用该两号）
- **例外表改语义**：`scripts/wiki_lint.py` 的 `OVERSIZE_ACK` 由「**欠拆页记账 · 只减不增**」→「**已批准例外 · 不拆页**」：`queries/rd05t-vendor-inquiry-2026-09-18`（203 · 对外可发送件 —— PDF 由本页生成，拆页就不成一封信）· `comparisons/xl330-vs-kpower-rd05t`（220 · 待 T-10 回函写入）；**只减不增保留**（要涨先在脚本里改记账值 = 显式记账）；`concepts/hat-solder-kit` 条目**删除**（已 ≤200）
- **校验**：`wiki_lint` 61 页 **0 error / 2 warning**（两条即例外页）· `refs_lint` 0 error · `lint_selftest`（「超记账值」探针改后仍命中首条例外页）
- **未动（交回 Human）**：`AGENTS.md`「记录约定」还写着「『已知超长 · 只减不增』的记账表 … 欠拆页记在 `tasks.md`」—— 措辞已落后于「已批准例外」语义，而**规则只有中立会话能改**，pm 不碰
- Updated: [[hat-solder-kit]]（399 → 194 行 · 重写）· [[tasks]]（T-08 → `完成`）· [[index]]（导航描述 · 标签表增 `assembly` `solder` · 页阈值补例外指针）· `scripts/wiki_lint.py` · `scripts/README.md` · `log.md`

## [2026-09-19] pm | 台账：T-03 退回待办；T-01 拆两件（装配 / 供电链路）

- **由来（Human）**：「T-03 放入 todo，T-01 拆成两个任务：整机装配、供电链路装配测试」
- **T-03 → `待办`**（原 `执行`）—— 新焊 3 块 HAT 属**台架等待间隙的填充工作**，不该占 WIP 名额；行文（验收 / 出处）未改，只换表
- **T-01 拆两条**（原「整机装配 ＋ 供电链路」验收里混了两类事：**不上电的机械装配** vs **要上电的电测**）
  - **T-01 整机装配**（留原号）—— 叠装顺序 ＋ 固定方式成文 · 线束走线与所需线长记录（与 [[board-interconnect]] 一致）· **不上电**试装一次不干涉电池 / 腿 / 线束
  - **T-13 供电链路装配测试**（**新号**，编号不复用）—— 母线 **6.0 V** 实测 ＋ 降压模块型号 / 额定 / 实测纹波（三种各一遍）· **上电须本人签**（红线 6）→ 落 [[bench-power-supply]]
