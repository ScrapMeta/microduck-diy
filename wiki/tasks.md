...
| **宿主工具链清理**（Windows 开发机 · 系统级） | `/microduck-pm` | ① 删 `C:\Python\Python38`（**1648 MB**，已无 `python.exe`，只剩孤立 `site-packages`）＋ 从**机器 PATH** 移除 `C:\Python\Python38\Scripts` / `C:\Python\Python38`（**机器 PATH 排在用户 PATH 之前**，坏 exe 抢 `tensorboard` / `huggingface-cli` / `modelscope`）② 删 `C:\Python\Python39`（92 MB）＋ 清 HKLM `PythonCore\3.9`（修 `py -3.9` 报错）③ `C:\Python\miniconda3`（**871 MB**）留删**待定** ④ 关商店 `python3.exe` 执行别名（GUI，人操作）。**均属破坏性 / 系统级 → 等 Human 逐项点头** | 2026-09-19 · [[local-workspace-layout]] §宿主工具链 |
