...
| 项 | 设定 |
|----|------|
| 电压 | **7.4 V**（对齐 NP-F；也可先 **6.0 V** 更贴 XL330 标称） |
| 电流限 | **1 A**（单只空载够；勿开到 10 A 做首通） |
| 进电 | J13 或 J14：**针1=GND · 针2=+BATT**；针3 先空 |
| 禁止 | 30 V、Type-C 与 `+BATT` 同时灌、一次挂多只 |
...
台供 7.4 V / 1 A → J13 针2/针1。叠 Zero，系统已能亮屏。

| # | 测 | 期望 | 失败 |
|---|-----|------|------|
| 2.1 | 台供电流（不接舵机） | 约 0.2–0.8 A（Zero 跑着） | 顶死 1 A → 短路，先别接舵机 |
| 2.2 | J13 **针2↔针1** | ≈ 台供电压 | 无压：座虚焊 / 没接到 `+BATT` |
| 2.3 | J4 **5V / 3V3** | ≈5.0 / ≈3.3 | 见 [[hat-solder-kit]] §4 |
| 2.4 | 再插 **1 只** 舵机，针2 仍≈7.4 V | 电流多 **0.1–0.3 A** | 电流暴涨 → 线序反 / 舵机损坏 |
