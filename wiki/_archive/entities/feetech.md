---
title: Feetech（飞特）
created: 2026-08-30
updated: 2026-09-08
type: entity
tags: [company, servo, feetech]
sources:
  - raw/transcripts/research-hardware-servos-2026-08.md
  - raw/articles/openmicroduck-joyandai-2026-09-04.md
confidence: high
---

# Feetech（飞特模型）

深圳串行总线舵机厂商（STS/SCS/HLS 等）。[[elec-rpi-robot-hat]] 硬件层可接飞特（需线序适配），但 Microduck 软件栈是 Dynamixel。

## 本 wiki 覆盖型号

- [[feetech-sts3032]] — 最贴 XL330 电气
- [[feetech-sts3215]] — 更大扭矩；BAM 已有模型
- [[feetech-hd-1910]] — 5–8.4 V TTL；开源小鸭预售款（OpenMicroDuck）
- [[feetech-hl-2909]] — 9–14 V 样机款；规格书同 HL-2915-C002
- [[feetech-hl-2915]] — 早期笔记口径（与规格书冲突时以 PDF 为准）
- [[feetech-hl-3915]] — 12V 铝壳恒力版

相关：[[xl330-vs-feetech-servos]] · [[openmicroduck]] · [[better-actuator-models-bam]]
