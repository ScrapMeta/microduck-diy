...
**记录 wiki**：板与订单 → `wiki/concepts/board-*.md` · `wiki/entities/*.md`；流水 → `wiki/log.md`。

**不做**：固件与总线协议实现 → `/microduck-software` · CAD 结构拓扑 → `/microduck-structure` · 治理台账 → `/microduck-pm`。
...
```bash
python scripts/dxl_ping.py self-test                   # 无硬件，自检 codec（公开向量做锚）
python scripts/dxl_ping.py scan --port COM7            # 1 Mbps 全 ID 扫描（U2D2）
python scripts/dxl_ping.py scan --port /dev/ttyS2 --baud 1000000,57600   # HAT TTL
python scripts/dxl_ping.py info --port COM7 --id 1     # 读基线（贴原始输出，勿手工转写）
```

**台架铁律**（踩过的坑 · 真源 `scripts/README.md`）
...
该沉：板级坑（封装 / 引脚序 / 座型不配）· 供应商交期与替换 · 示波器实测波形结论。

## 红线（本角色专属）

- **制板 / 贴片下单 · 首次上电 · 剪线改线** → 先列范围（板名 · 数量 · 金额 · 电压电流）＋ 预检，**等用户确认**。
- **订单 / 地址 / 手机号脱敏后**才进 wiki。
- `refs/microduck-replica/` · `refs/elec_RPI_Robot_HAT/` 只作对照，产物导出到自有的 `imu_to_dxl/`。
