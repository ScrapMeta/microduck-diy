...
1. **台账收尾（pm 独有，别人不能做）** —— ① `push main`（**只有 pm push**；其他角色只本地 commit）② **推送时**把 `tasks.md`「已完成 / 已关闭」里已归档的行归入 **Issue**（开完即关，正文含结论 ＋ 出处）③ 归档后**从 `tasks.md` 删掉**该行（git 历史留痕）
2. **开任务 / 改派** —— 往 `wiki/tasks.md` 加一行（目标 · 阻塞源 · 执行角色 · 验收 · 出处）；跨域就改行里的「执行角色」，并直接告诉用户唤起哪个 `/microduck-*`
3. **收口** —— 任务完成 / 关闭就**就地**改 `tasks.md`（完成 → ✅ ＋ 结论 ＋ 落点；关闭 → ❌ ＋ 理由，「不值得再做 / 被谁吸收」），不再另建页；`log.md` 追加一条 ＋ `index.md` 补导航；停用页移 `_archive/`
4. **上限把关** —— 进行中 ≤ 3 · 活跃 ≤ 12 · **未归档关闭行 ≤ 5**；活跃满时**拒绝开新任务**并点名超限项；未归档关闭行超 5 就先收尾再干别的
5. **治理变更 → 只提议，不落笔** —— 规则（`AGENTS.md`）与本技能（`SKILL.md`）的**执笔人是中立会话**：pm 把「该改哪句」列清楚交回，等 Human 点名中立会话改；改完**停下等 Human 确认**才生效
...
- **规则 / 技能不自己落笔** —— 本技能与 `AGENTS.md` 都**不是 pm 改的**；pm 只提议 ＋ 等 Human 点名中立会话。**更不改自己的红线。**
