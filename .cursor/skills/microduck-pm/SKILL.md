...
**读盘纪律** —— 这几个文件体量差 20 倍，读法要分开，否则一次收口就白烧几千行上下文：

- **`wiki/log.md` 只读尾部** —— 它**只追加 · 只会变长**（已到千行量级 / ~150 KB），**整读是本角色最贵的一笔浪费**。用 Read 的 `offset` **取负数**读尾部（如 `offset: -15`）；要翻旧账先 `Grep -n` 定位行号，再按 `offset` 读附近那几行。
- **`wiki/tasks.md` 整读** —— 不到百行，直接读全。
- **`wiki/index.md`** 是导航，按节读；**改了它才需要读全**。
