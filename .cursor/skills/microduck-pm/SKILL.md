...
1. **台账收尾（pm 独有，别人不能做）** —— ① `push main`（**只有 pm push**；其他角色只本地 commit）② **推送时**把 `tasks.md`「完成」里未归档的行**一批合成一个 Issue**（开完即关，正文含结论 ＋ 出处）③ 归档后**从 `tasks.md` 删掉该批行**（git 历史留痕）。**归档这个动作只记 `log.md`，不在 `tasks.md` 留占位行** —— 否则「记归档的那一行」自己又成了待归档的行
...
4. **不做数量把关** —— 台账不设上限，条数多不拒绝开任务；**太长时人工收拢**（`push main` → 把 `完成` 行一批归档 Issue 并从本页删掉）
